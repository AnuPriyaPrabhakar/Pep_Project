...
import com.ponsun.pep.accessPermission.rowmapper.AccessPermissionRowMapper;
...
public class AccessPermissionWritePlatformServiceImpl implements  AccessPermissionWritePlatformService {
    ...
    public List<AccessPermissionData> fetchAllAccessPermissionData(String uid){
        ...
        String whereClause =" WHERE a.id=b.mod_id AND c.mod_id=a.id AND c.mod_det_id=b.id AND c.uid=? AND c.status = 'A' AND a.id ORDER BY a.orderno ";
        Qry = Qry + whereClause;
