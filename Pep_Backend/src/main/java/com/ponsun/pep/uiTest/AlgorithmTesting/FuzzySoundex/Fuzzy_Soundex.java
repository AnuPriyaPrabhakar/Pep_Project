package com.ponsun.pep.uiTest.AlgorithmTesting.FuzzySoundex;
...
public class Fuzzy_Soundex {
    public static ArrayList<String> token;
    public static double getFuzzySoundx(String nametest1,String nametest2){
        Soundex soundex = new Soundex();

//        nametest1= soundex.encode(nametest1);
//        nametest2= soundex.encode(nametest2);
        ...
        for(int n1=0;n1<token1.size();n1++) {
            token1.set(n1,soundex.encode(token1.get(n1)));
        }
        for(int n2=0;n2<token2.size();n2++) {
            token2.set(n2,soundex.encode(token2.get(n2)));
        }
