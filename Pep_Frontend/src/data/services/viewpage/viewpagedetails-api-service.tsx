...
//1
  ...
  getPartyList = async (pepId: number) => {
    try {
      const response = await this.httpClientWrapper.get(`/api/v1/PartyDTO/getPartyDetails/{pepId}?pepId=${pepId}`);
      console.log("party", response);
      return response;
    } catch (error) {
      console.error('HttpClientWrapper get() error===', error);
      throw error; 
    }
  }
...
getPan = async (pan: string) => {
  try {
    const response = await this.httpClientWrapper.get(`/api/v1/PanApi?pan=${pan}`);
    return response;
  } catch (error) {
    console.error('Error fetching company details:', error);
    throw error;
