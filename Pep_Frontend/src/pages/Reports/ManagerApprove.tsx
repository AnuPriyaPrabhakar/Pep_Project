...
import ManagerApproveApiService from '../../data/services/Reports/ManagerApprove/managerapprove-api-service';
import { ManagerApproveData } from '../../data/services/Reports/ManagerApprove/managerapprove-payload';


interface ManagerApprovePayload {
    fromDate: string;
    toDate: string;
    name: string;
    count: string;
...
function ManagerApprove() {
    ...
    const managerApproveApiService = new ManagerApproveApiService();
    const [searchPerformed, setSearchPerformed] = useState(false);
    const [data, setData] = useState<ManagerApprovePayload[]>([]);
    ...
    const handleSearch = () => {
            ...
            managerApproveApiService.getCustomDate(startDateAsDate, endDateAsDate)
            .then((fetchedData: ManagerApproveData[]) => {
                setSearchPerformed(true);
                const transformedData: ManagerApprovePayload[] = fetchedData.map(entry => {
                    return {
    ...
    return (
        <>
          <h4 style={{ marginTop: '6%', marginLeft: '2%' }}>Manager Approval</h4>
            <Header />
        ...
        <CardContent>
                        ...
                        {data.length > 0 && (
                            <table className="table report-table">
                                ...
                                <tbody>
                                    {data.map((item: ManagerApprovePayload, index: number) => (
                                        <tr key={index}>
                                            <td>{index + 1}</td>
                                            <td>{item.name}</td>
                                            <td>{item.count}</td>
