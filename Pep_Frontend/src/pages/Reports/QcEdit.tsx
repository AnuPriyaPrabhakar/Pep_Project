...
import QcEditApiService from '../../data/services/Reports/QcEdit/qcedit-api-service';
import { QcEditData } from '../../data/services/Reports/QcEdit/qcedit-payload';


interface QcEditPayload {
    fromDate: string;
    toDate: string;
    name: string;
    count: string;
...
function QcEdit() {
    ...
    const qcEditApiService = new QcEditApiService();
    const [searchPerformed, setSearchPerformed] = useState(false);
    const [data, setData] = useState<QcEditPayload[]>([]);
    ...
    const handleSearch = () => {
            ...
            qcEditApiService.getCustomDate(startDateAsDate, endDateAsDate)
            .then((fetchedData: QcEditData[]) => {
                setSearchPerformed(true);
                const transformedData: QcEditPayload[] = fetchedData.map(entry => {
                    return {
    ...
    return (
        <>
          <h4 style={{ marginTop: '6%', marginLeft: '2%' }}>Qc Edit</h4>
            <Header />
        ...
        <CardContent>
                        ...
                        {data.length > 0 && (
                            <table className="table report-table">
                                ...
                                <tbody>
                                    {data.map((item: QcEditPayload, index: number) => (
                                        <tr key={index}>
                                            <td>{index + 1}</td>
                                            <td>{item.name}</td>
                                            <td>{item.count}</td>
