...
import { PublishedData } from '../../data/services/Reports/PublishedData/publisheddata-payload';
import PublishedApiService from '../../data/services/Reports/PublishedData/publisheddata-api-service';


interface PublishedDataPayload {
    fromDate: string;
    toDate: string;
    count: string;
}
...
function Published() {
    ...
    const publishedApiService = new PublishedApiService();
    const [searchPerformed, setSearchPerformed] = useState(false);
    const [data, setData] = useState<PublishedDataPayload[]>([]);
    ...
    const handleSearch = () => {
            ...
            publishedApiService.getCustomDate(startDateAsDate, endDateAsDate)
            .then((fetchedData: PublishedData[]) => {
                setSearchPerformed(true);
                const transformedData: PublishedDataPayload[] = fetchedData.map(entry => {
                    return {
    ...
    return (
        <>
          <h4 style={{ marginTop: '6%', marginLeft: '2%' }}>Published Data</h4>
            <Header />
        ...
        <CardContent>
                        ...
                        {data.length > 0 && (
                            <table className="table report-table">
                                ...
                                <tbody>
                                    {data.map((item: PublishedDataPayload, index: number) => (
                                        <tr key={index}>
                                            <td>{index + 1}</td>
                                            <td>{item.count}</td>
                                        </tr>
