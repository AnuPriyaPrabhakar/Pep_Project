...
import QcViewApiService from '../../data/services/Reports/QcView/qcview-api-service';
import { QcViewData } from '../../data/services/Reports/QcView/qcview-payload';

interface QcViewPayload {
    fromDate: string;
    toDate: string;
    name: string;
    count: string;
...
function QcView() {
    ...
    const qcViewApiService = new QcViewApiService();
    const [searchPerformed, setSearchPerformed] = useState(false);
    const [data, setData] = useState<QcViewPayload[]>([]);
    ...
    const handleSearch = () => {
            ...
            qcViewApiService.getCustomDate(startDateAsDate, endDateAsDate)
            .then((fetchedData: QcViewData[]) => {
                setSearchPerformed(true);
                const transformedData: QcViewPayload[] = fetchedData.map(entry => {
                    return {
    ...
    return (
        <>
          <h4 style={{ marginTop: '6%', marginLeft: '2%' }}>Qc View</h4>
            <Header />
        ...
        <CardContent>
                        ...
                        {data.length > 0 && (
                            <table className="table report-table">
                                ...
                                <tbody>
                                    {data.map((item: QcViewPayload, index: number) => (
                                        <tr key={index}>
                                            <td>{index + 1}</td>
                                            <td>{item.name}</td>
                                            <td>{item.count}</td>
